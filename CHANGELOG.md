## 1.3.0 — Приватная история посещений

- Добавлена скрытая защищённая страница `/admin/visits`, не связанная ссылками с публичным интерфейсом.
- Сохраняются время, приблизительные страна и город, тип устройства, ОС, браузер и открытый путь.
- Полный IP-адрес не хранится; HMAC используется только для минутной дедупликации одинакового открытия.
- Админка, API, медиа, редакторы, предварительные загрузки и известные боты не записываются.
- История автоматически ограничена 90 днями.
- Добавлены периоды, пагинация, пустое состояние, обработка ошибок, таблица для компьютера и карточки для телефона.
- Production-проверка подтвердила фоновую запись, дедупликацию, защиту авторской сессией и очистку контрольных данных.

## 1.2.0 — Фотографии страниц

- В книгу и дневник добавлено до шести фотографий на одну запись.
- Фотографии можно расположить перед текстом или после него и дополнить необязательной подписью.
- Поддерживаются JPEG, PNG, WebP, HEIC и HEIF размером до 15 МБ.
- Исходные фотографии безопасно обрабатываются, уменьшаются без обрезки и сохраняются в приватном Cloudflare R2.
- Добавлены защищённые API загрузки, изменения и удаления фотографий.
- При удалении страницы связанные изображения удаляются из базы и R2.
- Композиция чтения адаптирована для портретных, горизонтальных и квадратных фотографий без нарушения атмосферы книги.
- Выполнена реальная production-проверка создания, редактирования, перемещения, подписи и удаления фотографии.

## 1.1.1 — Возврат со страницы входа

- На странице авторского входа добавлена спокойная ссылка «Вернуться к книге».
- Случайный переход в закрытую часть больше не оставляет читателя без понятного пути назад.
